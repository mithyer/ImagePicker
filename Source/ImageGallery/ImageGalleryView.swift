...
open class ImageGalleryView: UIView {
  ...
  enum FetchAssetsState: Int {
    ...
    var didFetchedSomeAssets: Bool {
      return self.rawValue >= FetchAssetsState.fetchedSome.rawValue
    }
  }
  ...
  var imageLimit = 0
  var fetchAssetsState: FetchAssetsState = .prepare
  ...
  func updateNoImagesLabel() {
    if !self.fetchAssetsState.didFetchedSomeAssets {
      return
    }
  ...
  func fetchPhotos(_ completion: (() -> Void)? = nil) {
    self.fetchAssetsState = .fetchingSome
    AssetManager.fetch(withConfiguration: configuration, fetchAll: false) { assets, fetchedAll in
      self.fetchAssetsState = fetchedAll ? .fetchedAll : .fetchedSome;
...
extension ImageGalleryView: UICollectionViewDelegate {
  ...
  public func scrollViewDidScroll(_ scrollView: UIScrollView) {
    ...
    if (self.fetchAssetsState == .fetchedSome && bottomEdge >= scrollView.contentSize.width - scrollView.bounds.width/2) {
      self.fetchAssetsState = .fetchingAll
      AssetManager.fetch(withConfiguration: configuration, fetchAll: true) { assets, fetchedAll in
        self.fetchAssetsState = .fetchedAll
        self.assets.removeAll()
        self.assets.append(contentsOf: assets)
        self.collectionView.reloadData()
      }
...
extension ImageGalleryView: UICollectionViewDataSource {
  ...
  public func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
    ...
    let asset = assets[(indexPath as NSIndexPath).row]
    cell.configureCell(asset, shouldPreferLowRes: configuration.useLowResolutionPreviewImage, selected: self.selectedStack.containsAsset(asset))
    return cell
  }
}
